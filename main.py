...
# write_video = 'result5.avi'
write_video = None

camera = 'demo.avi' # 讀取檔案
# camera = 0            # 使用攝影機
...
#         cv2.line(img, (0, int(img.shape[0]/2)), (int(img.shape[1]), int(img.shape[0]/2)) , (0, 255, 255), 2) # Camera中心線左右
        ...
        cv2.putText(img, '{}'.format(distance_x), (10, 200), cv2.FONT_HERSHEY_TRIPLEX, 1, (0, 255, 255), 1, cv2.LINE_AA)
...
#         %matplotlib inline
    ...
    if write_video:
        out = cv2.VideoWriter(write_video, cv2.VideoWriter_fourcc(*'MJPG'), 25.0, (640, 480))
